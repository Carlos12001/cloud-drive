...
const FileInput = ({name, label, value, type, handleInputState, ...rest}) => {
    ...
    const handleUpload = () => {
        ...
        const storageRef = ref(
            storage,
            type === "audio" ? `/audio/${fileName}`:`/imagenes/${fileName}`
        );
    ...
    return(
        <div className={styles.container}>
            ...
            {type === "image" && value &&(
                <img
                    scr={typeof value === "string" ? value: URL.createObjectURL(value)}
                    alt="file"
            ...
            {progress === 100 &&(
                <div className={styles.progress_container}>
                    <img scr={check} alt="check circle" className={styles.check_img}/>
                </div>
            )}
        </div>
